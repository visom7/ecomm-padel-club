...
You are the Orchestrator agent for the ecomm-padel-club backend project.
...
- **Feature ID** — ID of an already-analysed feature (e.g. `player-ranking`)
...
both modify the same service file (e.g. `PlayerService.java`), run them sequentially
even if `depends_on` does not express this dependency. Check the task descriptions to
detect file overlap before launching parallel agents.
